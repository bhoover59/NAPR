loss_mechanisms <- function(df, intial){
  ...
  # Ground deposition ----------------------------------------------------------
  df$L_uptake_ground <- (df$v_HONO * df$HONO * initial$gamma_HONO_ground * 3600) / (4 * df$H)

  # Aerosol deposition/uptake --------------------------------------------------
  df$L_aerosol <- (df$v_HONO * df$HONO * initial$gamma_HONO_aerosol * initial$S_aerosol * 3600) / (4)

  # Photoenhanced ground deposition --------------------------------------------
  df$L_ground_light <- (df$v_HONO * df$HONO * df$gamma_HONO_ground * (df$RH/20) * (df$JNO2 / initial$beta)^3 * 3600) / (4 * df$H)

  # Photoenhanced aerosol deposition/uptake ------------------------------------
  df$L_aerosol_light <- (df$v_HONO * df$HONO * df$gamma_HONO_aerosol * (df$JNO2 / initial$beta) * initial$S_aerosol * 3600) / (4)
